...
const LOTE = parseInt(process.argv[3], 10) || 5;

if (!SELLER_ID) {
  console.error('❌ Debes indicar el SELLER_ID.');
  console.log('ℹ️ Uso: node verify-products-usa-batch.js SELLER_ID [CANTIDAD]');
  process.exit(1);
}

const DATA_DIR = path.join(__dirname, '..', 'data', 'vendors', SELLER_ID);
const CONSOLIDATED_JSON = path.join(DATA_DIR, 'all-products-consolidated.json');
...
(async () => {
  ...
  fs.writeFileSync(path.join(DATA_DIR, 'all-products-consolidated.csv'), csvContenido);
  console.log(`📄 CSV actualizado: ${path.join(DATA_DIR, 'all-products-consolidated.csv')}`);
